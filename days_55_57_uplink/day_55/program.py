...
# def write_post():
#     svc = BlogClient()
#     title = input("Please title your entry: ")
#     content = input("Please write some content: ")
#     published = datetime.datetime.now().isoformat()
#     resp = svc.create_new_entry(title=title,
#                                 content=content,
#                                 published=published)
#
#     print()
#     # print(f"Successfully created new entry with id = {resp.json().get('id)')}")
#
#
# def read_posts():
#     svc = BlogClient()
#     response = svc.get_all_entries()
#     response.raise_for_status()
#     entries = response.json()
#     for idx, e in enumerate(entries, 1):
#         print(f"{idx}: [{e.get('view_count')} views] -- {e.get('title')}")
#
#     print()
#     selected = input('Which would you like to view?')
#     selected_id = entries[int(selected) - 1].get('id')
#
#     response = svc.get_entry_by_id(selected_id)
#
#     entry = response.json()
#
#     print(f"Details for selected entry: {entry.get('title')}")
#     print(f"Written: {entry.get('published')}")
